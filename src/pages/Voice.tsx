import vocSpcBadge from '@assets/images/badges/SVG/voc_spc.svg';
import vocProBadge from '@assets/images/badges/SVG/voc_pro.svg';
...
const Voice = () => {
  ...
  return (
    <>
      <Header
        backgroundImage='bg-voice'
        title='Poly Voice Solutions'
        description='The Poly Voice track focuses on Poly Voice solutions such as Poly Trio, CCX phones and the underlying technologies of audio, VoIP, Call Servers and Device anagement. It is divided in 3 levels.'
      />
        ...
        <div>
          ...
          <SectionEnrollments>
            {data
              .filter(
                (course) =>
                  course.level === 'specialist' && course.category.voice
              )
              .map(({ ...rest }) => (
                <SectionCourseCard
                  {...rest}
          ...
          <SectionReward
            image={vocSpcBadge}
            description='Poly Voice Specialist Certification'
          />
        </div>
      </Container>
        ...
        <div>
          ...
          <SectionEnrollments>
            {data
              .filter(
                (course) =>
                  course.level === 'professional' && course.category.voice
              )
              .map(({ ...rest }) => (
                <SectionCourseCard
                  {...rest}
          ...
          <SectionReward
            image={vocProBadge}
            description='Poly Voice Professional Certification'
          />
        </div>
      </Container>
      ...
      <Container>
        <div>
          ...
          <SectionEnrollments>
            {data
              .filter(
                (course) => course.level === 'expert' && course.category.voice
              )
              .map(({ ...rest }) => (
                <SectionCourseCard
                  {...rest}
